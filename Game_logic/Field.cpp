...
void Field::clear_field(){
    for(int i = 0; i < field_size_y; i++){
        for(int j = 0; j < field_size_x; j++){
            field[i][j].set_type(FIELD_CELL_TYPE_NONE);
        }
    }
    ...
    for(int i = 0; i < field_size_x * field_size_y / 6 ; i++){
        add_wall();
    }
}
...
int Field::get_random_empty_cell(){
    ...
    for(int i = 0; i < field_size_y; i++){
        for(int j = 0; j < field_size_x; j++){
            if((field[i][j].get_type() == FIELD_CELL_TYPE_NONE) and (field[i][j].get_event() == nullptr)){
                empty_cell_count++;
            }
        }
    }
...
void Field::add_heal() {
    ...
    if(heal_position != -1){
        field[heal_position / field_size_x][heal_position % field_size_x].new_heal();
    }
}
...
void Field::add_trap() {
    ...
    if(trap_position != -1){
        field[trap_position / field_size_x][trap_position % field_size_x].new_trap();
    }
}
...
void Field::add_gun() {
    ...
    if(gun_position != -1){
        field[gun_position / field_size_x][gun_position % field_size_x].new_gun();
    }
}
...
void Field::add_key() {
    ...
    if(key_position != -1){
        field[key_position / field_size_x][key_position % field_size_x].new_key();
    }
}
...
void Field::add_exit() {
    ...
    if(exit_position != -1){
        field[exit_position / field_size_x][exit_position % field_size_x].new_exit();
    }
}
...
void Field::add_enemy() {
    ...
    if(enemy_position != -1){
        field[enemy_position / field_size_x][enemy_position % field_size_x].new_enemy();
    }
}
